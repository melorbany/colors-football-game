...
public class RevMobAndroid : RevMob {
  ...
  private AndroidJavaObject adUnitWrapperCall(string methodName, string placementId, string adUnit) {
	...
	public override RevMobBanner CreateBanner(int x, int y, int w, int h) {
		return (IsDevice()) ? new RevMobAndroidBanner(CurrentActivity(), CreateRevMobListener(this.gameObjectName, "Banner"),x, y, w, h, session) : null;
	}

	public override void ShowBanner(){
		this.session.Call("showBasicBanner",CurrentActivity(),CreateRevMobListener(this.gameObjectName, "Banner"));
	}

	public override void HideBanner() {
		this.session.Call("hideBasicBanner", CurrentActivity());
	}

	public override void ReleaseBanner(){
		this.session.Call("releaseBasicBanner",CurrentActivity());
	}
